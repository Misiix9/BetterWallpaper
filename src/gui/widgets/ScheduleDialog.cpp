...
void ScheduleDialog::createTimeSection() {
  ...
  // Add to parent (need to get parent from caller)
  GtkWidget *parent = gtk_widget_get_parent(m_nameEntry);
  if (parent) {
    parent = gtk_widget_get_parent(parent); // Get the innerBox
    if (parent) {
...
void ScheduleDialog::show() {
  ...
  emptyEntry.startTime = 9 * 60;     // 9:00 AM
  emptyEntry.daysOfWeek = 0b1111111; // All days
  show(emptyEntry);
}
