...
WorkshopView::~WorkshopView() { dismissDownloadToast(); }
...
void WorkshopView::setupBrowsePage() {
  ...
  // Search bar — matches library style: [SearchEntry] [SearchButton]
  // [LoginButton]
  ...
  g_signal_connect(
      m_prevButton, "clicked", G_CALLBACK(+[](GtkButton *, gpointer data) {
        auto *self = static_cast<WorkshopView *>(data);
        if (self->m_currentPage > 1)
          self->performSearch(self->m_lastQuery, self->m_currentPage - 1,
                              self->m_lastSort);
      }),
      this);
  ...
  g_signal_connect(
      m_nextButton, "clicked", G_CALLBACK(+[](GtkButton *, gpointer data) {
        auto *self = static_cast<WorkshopView *>(data);
  ...
  GtkWidget *placeholderDesc =
      gtk_label_new("Log in to your Steam account to browse\nand download "
                    "Workshop wallpapers.");
...
void WorkshopView::performSearch(const std::string &query, int page,
  ...
  GtkAdjustment *vadj = gtk_scrolled_window_get_vadjustment(
      GTK_SCROLLED_WINDOW(m_browseScrolled));
  ...
  std::thread([this, query, page, sort]() {
    auto result =
        bwp::steam::SteamService::getInstance().search(query, page, sort);
    ...
    g_idle_add(
        [](gpointer gdata) -> gboolean {
          ...
          gtk_progress_bar_set_fraction(
              GTK_PROGRESS_BAR(d->view->m_progressBar), 0.0);
...
void WorkshopView::updateBrowseGrid(
  ...
  bool showNsfw = bwp::config::ConfigManager::getInstance().get<bool>(
      "content.show_nsfw", false);
  ...
  for (const auto &item : items) {
    ...
    card->setDownloadCallback([this](const bwp::steam::WorkshopItem &item) {
      bwp::steam::DownloadQueue::getInstance().addToQueue(
          item.id, item.title, item.thumbnailUrl, item.author, item.votesUp);
      showDownloadProgress(item.title, 0.0f);
    });
    ...
    g_signal_connect(rightClick, "pressed",
                     G_CALLBACK(+[](GtkGestureClick *gesture, int /*n_press*/,
                                    double x, double y, gpointer data) {
                       auto *self = static_cast<WorkshopView *>(data);
                       auto *card =
                           static_cast<WorkshopCard *>(g_object_get_data(
                               G_OBJECT(gesture), "workshop-card"));
                       if (card) {
    ...
    g_object_set_data_full(
        G_OBJECT(cardWidget), "workshop-card-owner", card,
...
bool WorkshopView::isWorkshopItemDownloaded(
    const std::string &workshopId) const {
...
void WorkshopView::showCardContextMenu(WorkshopCard *card, double x, double y) {
  ...
  g_object_set_data_full(
      G_OBJECT(downloadAction), "dl-data", dlData,
  ...
  g_signal_connect(downloadAction, "activate",
                   G_CALLBACK(+[](GSimpleAction *action, GVariant *, gpointer) {
                     auto *d = static_cast<DownloadData *>(
                         g_object_get_data(G_OBJECT(action), "dl-data"));
                     bwp::steam::DownloadQueue::getInstance().addToQueue(
                         d->id, d->title);
  ...
  std::string *steamUrl = new std::string(
      "https://steamcommunity.com/sharedfiles/filedetails/?id=" + item.id);
  g_object_set_data_full(
      G_OBJECT(openSteamAction), "url", steamUrl,
      [](gpointer p) { delete static_cast<std::string *>(p); });
  g_signal_connect(
      openSteamAction, "activate",
      G_CALLBACK(+[](GSimpleAction *action, GVariant *, gpointer) {
        auto *url = static_cast<std::string *>(
            g_object_get_data(G_OBJECT(action), "url"));
  ...
  g_object_set_data_full(G_OBJECT(copyIdAction), "wid", idStr, [](gpointer p) {
    delete static_cast<std::string *>(p);
  });
  g_signal_connect(copyIdAction, "activate",
                   G_CALLBACK(+[](GSimpleAction *action, GVariant *, gpointer) {
                     auto *id = static_cast<std::string *>(
                         g_object_get_data(G_OBJECT(action), "wid"));
  ...
  gtk_widget_insert_action_group(popover, "workshop-ctx",
...
void WorkshopView::updatePagination() {
  ...
  if (hasResults) {
    std::string label = "Page " + std::to_string(m_currentPage) + " / " +
                        std::to_string(m_totalPages);
...
void WorkshopView::loadPopularWallpapers() {
  ...
  if (steam.getSteamUser().empty() || steam.getApiKey().empty())
    return;
...
void WorkshopView::showSteamLoginDialog(const std::string &prefillUser,
  ...
  auto *dialog = adw_alert_dialog_new(
      "Steam Login", "Enter your Steam credentials to enable downloads.");
  ...
  GtkWidget *twoFaLabel =
      gtk_label_new("Steam Guard / 2FA code (leave empty if not enabled)");
  ...
  adw_alert_dialog_set_response_appearance(ADW_ALERT_DIALOG(dialog), "login",
  ...
  // synchronously in the handler and never access the dialog from async
  // callbacks.
  ...
  auto loginResponseCb = +[](AdwAlertDialog *d, const char *response,
                             gpointer selfPtr) {
    auto *self = static_cast<WorkshopView *>(selfPtr);

    if (g_strcmp0(response, "login") != 0)
      return; // Cancel — dialog closes, done
    ...
    // From here on, only use `self`, `user`, `pass`, `twoFaCode` (safe value
    // copies). The dialog will be destroyed once this handler returns.
    ...
    if (user.empty() || pass.empty()) {
      ...
      g_idle_add(
          +[](gpointer data) -> gboolean {
            ...
            d->view->showSteamLoginDialog(
                d->user, "Username and password are required.");
            delete d;
            return G_SOURCE_REMOVE;
          },
          rd);
    ...
    bwp::steam::SteamService::getInstance().login(
        pass,
          ...
          g_idle_add(
              +[](gpointer data) -> gboolean {
                ...
                if (r->success) {
                  ...
                  if (bwp::steam::SteamService::getInstance()
                          .getApiKey()
                          .empty())
                    r->view->showApiKeyDialog();
                } else {
                  // Clear user and reopen dialog with error + username
                  // prefilled
...
void WorkshopView::showSteam2FADialog(const std::string &errorMsg) {
  ...
  adw_alert_dialog_set_response_appearance(ADW_ALERT_DIALOG(dialog), "submit",
                                           ADW_RESPONSE_SUGGESTED);
  ...
  auto twoFAResponseCb = +[](AdwAlertDialog *d, const char *response,
                             gpointer selfPtr) {
    auto *self = static_cast<WorkshopView *>(selfPtr);
    ...
    bwp::steam::SteamService::getInstance().submit2FA(
        code, [self](bool success, const std::string &msg) {
...
void WorkshopView::refreshLoginState() {
  ...
  // On first call, if user is saved, attempt auto-login via steamcmd cached
  // credentials
        ...
        [data](bool success, const std::string &msg) {
          ...
          g_idle_add(
              +[](gpointer ptr) -> gboolean {
                ...
                if (!r->success) {
                  LOG_INFO("Steam auto-login failed: " + r->msg +
                           " — clearing saved user");
                  bwp::steam::SteamService::getInstance().setSteamUser("");
                  r->view->refreshLoginState();
                } else {
  ...
  if (loggedIn) {
    ...
    if (m_placeholderBox)
      gtk_widget_set_visible(m_placeholderBox, TRUE);
    if (m_browseScrolled)
      gtk_widget_set_visible(m_browseScrolled, FALSE);
    if (m_paginationBox)
      gtk_widget_set_visible(m_paginationBox, FALSE);
  }
...
void WorkshopView::showProfileMenu() {
  ...
  gtk_widget_insert_action_group(m_profilePopover, "workshop",
                                 G_ACTION_GROUP(group));
  g_object_unref(group);
  g_object_unref(menu);
...
void WorkshopView::showLogoutDialog() {
  ...
  GtkWindow *window = GTK_IS_WINDOW(root) ? GTK_WINDOW(root) : nullptr;
  auto *dialog = adw_alert_dialog_new(
      "Logout", "This will forget your saved Steam account and API key.\nYou "
                "will need to log in again to download wallpapers.");
  adw_alert_dialog_add_response(ADW_ALERT_DIALOG(dialog), "cancel", "Cancel");
  adw_alert_dialog_add_response(ADW_ALERT_DIALOG(dialog), "logout", "Logout");
  adw_alert_dialog_set_response_appearance(ADW_ALERT_DIALOG(dialog), "logout",
                                           ADW_RESPONSE_DESTRUCTIVE);
  adw_alert_dialog_set_default_response(ADW_ALERT_DIALOG(dialog), "cancel");
  g_signal_connect(
      dialog, "response",
      G_CALLBACK(
          +[](AdwAlertDialog *d, const char *response, gpointer selfPtr) {
            auto *self = static_cast<WorkshopView *>(selfPtr);
            if (g_strcmp0(response, "logout") == 0) {
              ...
              self->refreshLoginState(); // Will show placeholder and hide
                                         // browse area
...
void WorkshopView::showApiKeyDialog() {
  ...
  gtk_entry_set_placeholder_text(GTK_ENTRY(keyEntry),
                                 "Paste Key Here (e.g. 12345ABC...)");
  ...
  g_signal_connect(
      ...
      G_CALLBACK(+[](AdwAlertDialog *d, const char *response,
        ...
        if (g_strcmp0(response, "save") == 0) {
          GtkWidget *entry =
              GTK_WIDGET(g_object_get_data(G_OBJECT(d), "key_entry"));
          ...
          if (!key.empty()) {
            ...
            req.type = bwp::core::utils::ToastType::Success;
            bwp::core::utils::ToastManager::getInstance().showToast(req);
            self->refreshLoginState(); // Just to use 'self' and suppress
                                       // warning, though technically refreshes
                                       // login state which is harmless
          }
        }
        adw_dialog_close(ADW_DIALOG(d));
      }),
...
void WorkshopView::setupDownloadCallbacks() {
  ...
  dq.setProgressCallback([this](const std::string &workshopId,
                                const bwp::steam::DownloadProgress &prog) {
    ...
    auto *pd = new ProgData{this, workshopId, prog.title, prog.percent};
    g_idle_add(
        +[](gpointer data) -> gboolean {
          auto *d = static_cast<ProgData *>(data);
          d->view->showDownloadProgress(d->title, d->percent);

          // Forward to DownloadIndicator
          auto *app = g_application_get_default();
          if (app) {
            auto *win = gtk_application_get_active_window(GTK_APPLICATION(app));
            if (win) {
              // Access MainWindow's download indicator
              auto *mainWin = static_cast<bwp::gui::MainWindow *>(
                  g_object_get_data(G_OBJECT(win), "main-window"));
              if (mainWin && mainWin->getDownloadIndicator())
                mainWin->getDownloadIndicator()->updateProgress(
                    d->workshopId, d->title, d->percent);
            }
          }
          delete d;
          return G_SOURCE_REMOVE;
  ...
  dq.setCompleteCallback([this](const std::string &workshopId, bool success,
          ...
          if (app) {
            ...
            if (win) {
              auto *mainWin = static_cast<bwp::gui::MainWindow *>(
                  g_object_get_data(G_OBJECT(win), "main-window"));
              if (mainWin && mainWin->getDownloadIndicator())
                mainWin->getDownloadIndicator()->downloadComplete(d->workshopId,
                                                                  d->success);
            }
          }
...
void WorkshopView::showDownloadProgress(const std::string &title,
                                        float percent) {
  ...
  if (!win)
    return;
  GtkWidget *content =
      adw_application_window_get_content(ADW_APPLICATION_WINDOW(win));
  ...
  if (m_downloadToast && m_downloadProgressLabel) {
    ...
    std::string markup =
        "<span color='#93C5FD'>" + std::string(escaped) + "</span>";
    ...
    gtk_label_set_markup(GTK_LABEL(m_downloadProgressLabel), markup.c_str());
    g_free(escaped);
    gtk_label_set_ellipsize(GTK_LABEL(m_downloadProgressLabel),
                            PANGO_ELLIPSIZE_END);
...
void WorkshopView::showDownloadComplete(const std::string &title,
                                        bool success) {
