/*
 * ═══════════════════════════════════════════════════════════════════════════════
 * FLUID INTELLIGENCE SETUP WIZARD
 * ═══════════════════════════════════════════════════════════════════════════════
 * 
 * A living, breathing liquid interface that guides users through first-run setup.
 * Everything morphs. Nothing is static. This is not a form—it's a journey.
 * 
 * Design Principles:
 * - Spring physics for all motion (no linear easing)
 * - Continuous canvas flow (no page transitions)
 * - Kinetic typography (text as motion)
 * - Progressive disclosure (show only what's needed)
 * - Deep space aesthetics (blur, glow, parallax)
 */
...
struct SpringConfig {
    double mass = 1.0;       // Object mass
    double tension = 170.0;  // Spring stiffness (higher = snappier)
    double friction = 26.0;  // Damping (higher = less bouncy)
...
public:
    ...
    bool isAtRest() const;
    void tick(double dt); // dt in seconds
...
enum class FluidPhase {
    ...
    Emergence,      // Logo/Brand coalescing from particles
    Welcome,        // The greeting - big question format
    MonitorSense,   // Detecting/selecting monitors
    LibraryPulse,   // Finding/selecting wallpaper sources
    Completion,     // Success burst and launch
    Dissolve        // Fading into main app
};
...
// ═══════════════════════════════════════════════════════════════════════════════
// PARTICLE SYSTEM (For emergence/success effects)
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// THE FLUID CONTAINER - The Living Canvas
// ═══════════════════════════════════════════════════════════════════════════════
