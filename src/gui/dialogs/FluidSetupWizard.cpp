...
void SpringValue::setImmediate(double value) {
  m_current = value;
  m_target = value;
  m_velocity = 0.0;
}
...
void SpringValue::tick(double dt) {
  ...
  double acceleration =
      (-m_config.tension * displacement - m_config.friction * m_velocity) /
      m_config.mass;
...
void KineticText::setVisible(bool visible) {
  m_visible = visible;
  m_time = 0.0;
}
...
void KineticText::render(cairo_t *cr, double baseX, double baseY) {
  ...
  for (size_t i = 0; i < m_letters.size(); i++) {
    ...
    if (letter.opacity.getValue() < 0.01)
      continue;
...
FluidSetupWizard::FluidSetupWizard(GtkWindow *parent) {
  ...
  // Self-delete when window is closed without completing the wizard
  m_destroyHandlerId = g_signal_connect(
      m_window, "destroy", G_CALLBACK(+[](GtkWidget *, gpointer data) {
        delete static_cast<FluidSetupWizard *>(data);
      }),
      this);
  setupUi();
  detectMonitors();
...
void FluidSetupWizard::setupInputWidgets() {
  ...
  g_signal_connect(m_primaryButton, "clicked", G_CALLBACK(onPrimaryClick),
                   this);
  ...
  g_signal_connect(m_secondaryButton, "clicked", G_CALLBACK(onSecondaryClick),
                   this);
...
void FluidSetupWizard::close() {
  ...
  // Disconnect self-delete handler before destroying window (avoids
  // double-delete)
  if (m_destroyHandlerId > 0) {
    g_signal_handler_disconnect(m_window, m_destroyHandlerId);
    m_destroyHandlerId = 0;
  }
...
void FluidSetupWizard::startAnimationLoop() {
  if (m_tickSourceId == 0) {
    m_tickSourceId =
        gtk_widget_add_tick_callback(m_window, onTick, this, nullptr);
...
void FluidSetupWizard::onPhaseEnter(FluidPhase phase) {
  ...
  case FluidPhase::Welcome:
    ...
    m_subtitleText =
        std::make_unique<KineticText>("Your desktop, reimagined.", 0.025);
  ...
  case FluidPhase::LibraryPulse:
    m_questionText =
        std::make_unique<KineticText>("Where are your wallpapers?", 0.025);
    m_questionText->setVisible(true);
...
void FluidSetupWizard::onTick(double dt) {
  ...
  if (m_titleText)
    m_titleText->tick(dt);
  if (m_subtitleText)
    m_subtitleText->tick(dt);
  if (m_questionText)
    m_questionText->tick(dt);
  m_particles.tick(dt);
...
void FluidSetupWizard::renderBackground(cairo_t *cr, int width, int height) {
  ...
  cairo_pattern_t *gradient = cairo_pattern_create_radial(
      width / 2.0, height / 2.0, 0, width / 2.0, height / 2.0, width * 0.6);
...
void FluidSetupWizard::renderVignette(cairo_t *cr, int width, int height) {
  cairo_pattern_t *vignette =
      cairo_pattern_create_radial(width / 2.0, height / 2.0, width * 0.25,
...
void FluidSetupWizard::renderGlow(cairo_t *cr, int width, int height) {
  ...
  cairo_pattern_t *glow = cairo_pattern_create_radial(
      width / 2.0, height / 2.0, 0, width / 2.0, height / 2.0, 200);
...
void FluidSetupWizard::onDraw(GtkDrawingArea *, cairo_t *cr, int width,
                              int height, gpointer data) {
  static_cast<FluidSetupWizard *>(data)->render(cr, width, height);
}
gboolean FluidSetupWizard::onTick(GtkWidget *, GdkFrameClock *clock,
                                  gpointer data) {
...
void FluidSetupWizard::onPrimaryClick(GtkButton *, gpointer data) {
  ...
  case FluidPhase::LibraryPulse: {
    ...
    gtk_file_dialog_select_folder(
        dialog, GTK_WINDOW(self->m_window), nullptr,
...
gboolean FluidSetupWizard::onKeyPress(GtkEventControllerKey *, guint keyval,
