...
#include <windows.h>
#include <iostream>
#include <thread> // Added thread include
...
bool NamedPipeService::initialize() {
    m_running = true;
    // Launch listener in separate thread
    std::thread([this]() {
        this->listenLoop();
    }).detach(); 
    return true;
    ...
    while (m_running) {
        ...
        if (connected) {
            ...
            if (ReadFile(hPipe, buffer, sizeof(buffer) - 1, &bytesRead, NULL)) {
                ...
                else if (message == "PAUSE") { if(m_pauseHandler) m_pauseHandler("primary"); } // Or toggle
