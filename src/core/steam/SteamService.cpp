...
void SteamService::initialize() {
  ...
  LOG_INFO("Steam API key loaded: " +
           std::string(key.empty() ? "not set" : "present"));
}
...
std::string SteamService::getSteamUser() const {
  return bwp::config::ConfigManager::getInstance().get<std::string>(
      "steam_user", "");
}
...
std::string SteamService::getApiKey() const {
  std::string stored =
      bwp::config::ConfigManager::getInstance().get<std::string>(
          "steam_api_key", "");
  if (stored.empty())
    return "";
  return bwp::utils::StringUtils::trim(
      bwp::utils::StringUtils::base64Decode(stored));
}
...
void SteamService::tryAutoLogin(
    std::function<void(bool success, const std::string &msg)> callback) {
  std::string user = getSteamUser();
  if (user.empty()) {
    if (callback)
      callback(false, "No Steam User configured");
    return;
  }
...
void SteamService::downloadWallpaper(const std::string &workshopId,
  ...
  m_worker.download(
      workshopId, progress, [this, workshopId, complete](bool success) {
        if (success) {
          // Trigger Library Reload
          LOG_INFO("Download complete for " + workshopId +
                   ". Reloading library...");

          // Where does steamcmd put files?
          // Usually:
          // ~/.local/share/Steam/steamapps/workshop/content/431960/<id> or
          // ~/Steam/... The LibraryScanner scans these locations. So calling
          // reload() typically finds them.
          ...
          if (!found) {
            LOG_WARN("Could not automatically locate downloaded item: " +
                     workshopId);
          }
        }
        if (complete)
...
void SteamService::cancelDownload() { m_worker.cancel(); }
