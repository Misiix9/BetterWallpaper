...
void TransitionEngine::startWithEasing(cairo_surface_t *from,
                                       ...
                                       FinishCallback callback) {
  stop(); // Clean up any existing transition

  // Use preloaded surface if available and matches
  ...
  m_effect = effect;
  m_durationMs = std::max(1L, durationMs); // Minimum 1ms to avoid division by 0
