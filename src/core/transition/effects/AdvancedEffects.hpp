...
/**
 * Circle reveal transition - new wallpaper revealed from expanding circle.
 */
...
private:
  double m_originXRatio = 0.5; // Center by default
  double m_originYRatio = 0.5;
};

/**
 * Square reveal transition - new wallpaper revealed from expanding square.
 */
...
private:
  ...
  double m_originYRatio = 0.5;
  double m_cornerRadius = 0.0; // 0 = sharp corners
};

/**
 * Dissolve transition - pixelated random reveal.
 */
...
/**
 * Zoom transition - zoom into old, reveal new.
 */
...
private:
  double m_zoomFactor = 1.5; // How much to zoom before switching
};

/**
 * Morph/Blend transition - smooth color blending between wallpapers.
 */
...
/**
 * Enhanced wipe with angle support.
 */
...
private:
  double m_angleDegrees = 0.0; // 0 = horizontal left-to-right
  bool m_softEdge = false;
  double m_edgeWidth = 50.0; // Gradient width for soft edge
};

/**
 * Pixelate transition - pixelate out, pixelate in.
 */
...
/**
 * Blinds transition - horizontal or vertical blinds effect.
 */
...
private:
  int m_blindCount = 10;
  bool m_vertical = false; // false = horizontal blinds
};
